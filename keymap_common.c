...
uint16_t keycode_at_keymap_location(uint8_t layer, uint8_t row, uint8_t col) {
    switch (current_mode) {
        case MODE_1:
            return pgm_read_word(&mode1_keymaps[layer][row][col]);
        case MODE_2:
            return pgm_read_word(&mode2_keymaps[layer][row][col]);
        // Add more modes if needed
        default:
            return pgm_read_word(&mode1_keymaps[layer][row][col]); // Default to MODE_1 keymap
    }
#ifdef ENCODER_MAP_ENABLE
